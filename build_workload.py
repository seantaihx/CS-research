...
def build_workloads_low_cpu_overlap(benchmark_data):
    ...
    TIMESTEP = 5
    TOTAL_CPUS = 32
    CPU_THRESHOLD = 8   # only prefer jobs using <= 8 CPUs

    all_workloads = []

    # keep only jobs with valid cpu/memory traces and small cpu count
    low_cpu_jobs = []
    for job in all_jobs:
        num_cpus = int(job.get("num_cpus", 0))
        cpu_util = job.get("cpu_util_percent", [])
        mem_util = job.get("memory_util_percent", [])

        if num_cpus <= 0:
            continue
        if num_cpus > CPU_THRESHOLD:
            continue
        if len(cpu_util) == 0 or len(mem_util) == 0:
            continue

        low_cpu_jobs.append(job)

    print("Number of low CPU candidate jobs:", len(low_cpu_jobs))
    ...
    for wi in range(NUM_WORKLOADS):
        ...
        # if not enough low-cpu jobs, sample from all valid jobs instead
        if len(low_cpu_jobs) >= JOBS_PER_WORKLOAD:
            selected_jobs = random.sample(low_cpu_jobs, JOBS_PER_WORKLOAD)
        else:
            valid_jobs = []
            for job in all_jobs:
                num_cpus = int(job.get("num_cpus", 0))
            ...
            selected_jobs = random.sample(valid_jobs, JOBS_PER_WORKLOAD)
    ...
    with open("benchmark_low_cpu_overlap.json", "w") as f:
...
def build_workloads(benchmark_data):
    ...
    with open("benchmark_new5.json", "w") as f:
        json.dump(all_workloads, f, indent=2)
