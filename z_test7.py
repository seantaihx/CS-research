...
for tL, tR, k in intervals_node1: # line 131
    if k == 2: # 2 overlapping jobs
        ...
        s = ica.inverse_transform(s)
        
        # X = S * A^T
        # S = X * (A^T)^-1
        # so each column of S is one component (un-mixed signal)
        c0 = pd.Series(s[:,0], index=mixed_matrix.index)
        c1 = pd.Series(s[:, 1] , index=mixed_matrix.index)
        ...
        m = (cpu1.index >= pd.to_datetime(tL, unit='s')) & (cpu1.index < pd.to_datetime
        (tR, unit='s'))
        s = cpu1.loc[m]
        if s.empty:
            continue
        ...
        c0 = pd.Series(s[:,0] , index=mixed_matrix.index)
        c1 = pd.Series(s[:, 1] , index=mixed_matrix.index)
        c2 = pd.Series(s[:, 2] , index=mixed_matrix.index)
...
# -----------------------Plotting----------------------
# Remove zeros for plotting, line 160
...
plt.tight_layout()
plt.show()
plt.savefig("sep_sig11.png", dpi=300)