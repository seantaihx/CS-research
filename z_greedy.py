...
# === BUILD TASK TABLE ===
    ...
    for node, ts in per_node_timestamps.items():
        ...
        plt.legend()
        plt.tight_layout()
        # SAVE FLAT: workload[i]greedy_(nodename).png
        plt.savefig(f"workload[{i}]greedy_({node}).png", dpi=300)
        plt.close()
    MSE = totalMSE/count
    ...
    for node, ts in per_node_timestamps.items():
        ...
        plt.tight_layout()
        # SAVE FLAT: workload[i]greedy_alltasks_(nodename).png
        out_file = f"workload[{i}]greedy_alltasks_({node}).png"
        plt.savefig(out_file)
        plt.close()

    print(f"✅ Multi-task plots saved flat in current directory")
    return MSE, MAPE
...
if __name__ == "__main__":
    ...
    # --------- ONLY ONE WORKLOAD ----------
    i = 1  # <--- set the single workload index you want to run
    singleMSE, singleMAPE = separate_utilization_per_workload(i)
    print(f"MSE = {singleMSE}")
    print(f"MAPE = {singleMAPE}")