...
for tL, tR, k in intervals_node1:
    if k == 2:
        ...
        separated = ica.fit_transform(mixed_matrix.values)  # shape: (T_eff, 2)

        # write back on valid timestamps (after shift/dropna)
        sep1_n1.loc[mixed_matrix.index] = separated[:, 0]
        sep2_n1.loc[mixed_matrix.index] = separated[:, 1]
        ...
        separated = ica.fit_transform(mixed_matrix.values)  # shape: (T_eff, 3)

        sep1_n1.loc[mixed_matrix.index] = separated[:, 0]
        sep2_n1.loc[mixed_matrix.index] = separated[:, 1]
        sep3_n1.loc[mixed_matrix.index] = separated[:, 2]
...
for tL, tR, k in intervals_node2:
    if k == 2:
        ...
        ica = FastICA(n_components=2, whiten='arbitrary-variance', random_state=42)
        separated = ica.fit_transform(mixed_matrix.values)

        sep1_n2.loc[mixed_matrix.index] = separated[:, 0]
        sep2_n2.loc[mixed_matrix.index] = separated[:, 1]
        ...
        sep1_n2.loc[mixed_matrix.index] = separated[:, 0]
        sep2_n2.loc[mixed_matrix.index] = separated[:, 1]
        sep3_n2.loc[mixed_matrix.index] = separated[:, 2]
