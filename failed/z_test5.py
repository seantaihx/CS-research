...
# --- write back separated components ---
        ...
        baseline_seg = pd.Series(m[0], index=X.index)
        base_n1.update(baseline_seg)
        
        
        # after computing contrib[...] and baseline_seg = pd.Series(m[0], index=X.index)

# --- proportional stacking (local to this interval) ---
        residual = X['lag_0'] - baseline_seg
        weights = [contrib[i].clip(lower=0.0) for i in range(len(contrib))]
        pos_sum = sum(weights)
        if (pos_sum.abs() < 1e-12).any():
            weights = [contrib[i].abs() for i in range(len(contrib))]
            pos_sum = sum(weights)
        eps = 1e-12
        stacked = []
        for i in range(len(contrib)):
            wi = weights[i]
            si = residual * (wi / (pos_sum + eps))
            si = si.fillna(0.0)
            stacked.append(si)

# --- update node1 signals with stacked values ---
        if k == 2:
            sep1_n1.update(stacked[0])
            sep2_n1.update(stacked[1])
        elif k == 3:
            sep1_n1.update(stacked[0])
            sep2_n1.update(stacked[1])
            sep3_n1.update(stacked[2])
...
recon_n1 = base_n1 + sep1_n1 + sep2_n1 + sep3_n1
recon_n2 = base_n2 + sep1_n2 + sep2_n2 + sep3_n2

print("MAE node1:", (recon_n1 - cpu1).abs().dropna().mean())
print("MAE node2:", (recon_n2 - cpu2).abs().dropna().mean())
...
ax1.plot(recon_n1.index, recon_n1.values, '--', linewidth=1.5, label="Reconstruction")
...
ax2.plot(recon_n2.index, recon_n2.values, '--', linewidth=1.5, label="Reconstruction")
...
plt.tight_layout()
plt.show()
plt.savefig("zzzzz11.png", dpi=300)