...
for i in range(25):
    ...
    # settings
    max_outer = 50        # passes over blocks
    ...
    if n1 > 0:
        ...
        for _ in range(max_outer):
            for blk in blocks1:
                ...
                r = b1 - y1                 # residual for this bloc
                # recon = Ax = Anewxnew + Aoldxold
                # => r = b - Aoldxold = Anewxnew
                reg = LinearRegression(positive=True, fit_intercept=False)
                reg.fit(Ablk, r)                 # X=Ablk (T x nb), y=r (T,)
                xblk = reg.coef_.astype(float)   # shape (nb,)
...
'''   # ---------- plot ----------
    ...
    plt.tight_layout()
    plt.savefig(f"sep_nnls_{i}.png", dpi=300)
    plt.close()
'''