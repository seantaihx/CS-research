...
def plot_benchmark_cdf(
    results_nnls,
    results_gm,
    *,
    p_tail=95,
    title_prefix="CDF across workloads",
    xlabel_cpu="CPU Utilization",
    xlabel_mem="Memory Utilization",
    save_path=None,
    show=True,
):
    actual_cpu = []
    ...
    # -------- collect workload-level NNLS + actual --------
    ...
    # -------- collect workload-level GM reconstructed --------
    ...
    # --- CPU CDF ---
    x1, y1 = _cdf_xy(actual_cpu)
    x2, y2 = _cdf_xy(nnls_cpu)
    x3, y3 = _cdf_xy(gm_cpu)

    axes[0].plot(x1, y1, label="Original")
    axes[0].plot(x2, y2, label="NNLS")
    axes[0].plot(x3, y3, label="Greedy Mean")

    for label, values in [("Original", actual_cpu), ("NNLS", nnls_cpu), ("Greedy Mean", gm_cpu)]:
    ...
    cpu_p50_actual, cpu_p95_actual = _percentile(actual_cpu, 50), _percentile(actual_cpu, p_tail)
    cpu_p50_nnls, cpu_p95_nnls = _percentile(nnls_cpu, 50), _percentile(nnls_cpu, p_tail)
    cpu_p50_gm, cpu_p95_gm = _percentile(gm_cpu, 50), _percentile(gm_cpu, p_tail)

    print("CPU Utilization percentiles")
    print(f"  Original:   p50={cpu_p50_actual:.6g}, p{p_tail}={cpu_p95_actual:.6g}")
    print(f"  NNLS:       p50={cpu_p50_nnls:.6g}, p{p_tail}={cpu_p95_nnls:.6g}")
    print(f"  GreedyMean: p50={cpu_p50_gm:.6g}, p{p_tail}={cpu_p95_gm:.6g}")
    if np.isfinite(cpu_p95_nnls) and cpu_p95_nnls > 0 and np.isfinite(cpu_p95_gm):
        print(f"  Tail ratio (GreedyMean/NNLS) at p{p_tail}: {cpu_p95_gm / cpu_p95_nnls:.3g}")

    # --- Memory CDF ---
    x1, y1 = _cdf_xy(actual_mem)
    x2, y2 = _cdf_xy(nnls_mem)
    x3, y3 = _cdf_xy(gm_mem)

    axes[1].plot(x1, y1, label="Original")
    axes[1].plot(x2, y2, label="NNLS")
    axes[1].plot(x3, y3, label="Greedy Mean")

    for label, values in [("Original", actual_mem), ("NNLS", nnls_mem), ("Greedy Mean", gm_mem)]:
        p50 = _percentile(values, 50)
        pt = _percentile(values, p_tail)
        if np.isfinite(p50):
    ...
    mem_p50_actual, mem_p95_actual = _percentile(actual_mem, 50), _percentile(actual_mem, p_tail)
    mem_p50_nnls, mem_p95_nnls = _percentile(nnls_mem, 50), _percentile(nnls_mem, p_tail)
    mem_p50_gm, mem_p95_gm = _percentile(gm_mem, 50), _percentile(gm_mem, p_tail)

    print("\nMemory Utilization percentiles")
    print(f"  Original:   p50={mem_p50_actual:.6g}, p{p_tail}={mem_p95_actual:.6g}")
    print(f"  NNLS:       p50={mem_p50_nnls:.6g}, p{p_tail}={mem_p95_nnls:.6g}")
    print(f"  GreedyMean: p50={mem_p50_gm:.6g}, p{p_tail}={mem_p95_gm:.6g}")
    if np.isfinite(mem_p95_nnls) and mem_p95_nnls > 0 and np.isfinite(mem_p95_gm):
    ...
    return {
        "cpu": {
            "original_p50": cpu_p50_actual,
            f"original_p{p_tail}": cpu_p95_actual,
            "nnls_p50": cpu_p50_nnls,
            f"nnls_p{p_tail}": cpu_p95_nnls,
            "gm_p50": cpu_p50_gm,
            f"gm_p{p_tail}": cpu_p95_gm
        },
        "mem": {
            "original_p50": mem_p50_actual,
            f"original_p{p_tail}": mem_p95_actual,
            "nnls_p50": mem_p50_nnls,
            f"nnls_p{p_tail}": mem_p95_nnls,
            "gm_p50": mem_p50_gm,
            f"gm_p{p_tail}": mem_p95_gm
        }
    }
...
if __name__ == "__main__":
    ...
    cdf_stats = plot_benchmark_cdf(
        ...
        results_gm,
        p_tail=95,
        title_prefix="Utilization CDF across workloads",
        xlabel_cpu="CPU Utilization",
        xlabel_mem="Memory Utilization",
